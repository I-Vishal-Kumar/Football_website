...
selectAll('.profile_popup_btns').forEach((item, i) => {
  item.addEventListener('click' , ()=>{
    if(i !== 4){
      ...
      window.open('https://t.me/Chelseafbcs', '_blank');
    }
  })
});
...
var swiper = new Swiper(".mySwiper", {
  ...
  on: {
    slideChange: function () {
      ...
      const values = [[1000,1200] , [1000 , 3500] , [1000,8500] , [1000,30000] , [1000,50000]];

      const upgrade_values = [['2600','7999'] , ['8000' , '16999'] , ['17000','52999'] , ['53000','104999'] , ['105000','208999']];
...
function load_bet_box() {
  ...
  item.addEventListener('click' , ()=>{
    ...
    select('#final_league_id').innerText = league_id;
...
const search = ()=>{
  ...
  const search_parameter = document.querySelector('#search_box').value.toUpperCase();
  const bet_box = document.querySelector('#matches_box');
  const team_a = document.querySelectorAll('.search_criteria');

  for(let i = 0; i < team_a.length; i++){
     let match = matches[i].querySelector('.search_criteria');
     
     if(match){
        let text_value = match.textContent || match.innerHTML 
        if(text_value.toUpperCase().indexOf(search_parameter) > -1){
...
document.querySelector('#search_box').addEventListener('keyup' , search);
...
function create_match(data){
  ...
  <h3 id = "final_league_name">${data['league']}</h3>
    <div class="div2">
        <span><h3 class='search_criteria' id='initial_team_a'>${data['team_a']}</h3><h3 id='initial_team_b'>${data['team_b']}</h3></span>
        <span class="vs_logo"></span>
        <span><h4>Today</h4> <h4 id='initial_time'>${date.getHours()}:${date.getMinutes()}</h4></span>
    </div>`
...
// --------------------------- this function will save the key value pairs of percentages in local storage------------

localStorage.clear();

function storeDataInLocalStorage(key, valueArray) {
  // Convert the array to a string before storing it
  const valueString = JSON.stringify(valueArray);

  // Store the key-value pair in local storage
  localStorage.setItem(key.toString(), valueString);
}
